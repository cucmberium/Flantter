...
namespace Flantter.MilkyWay.Models.Apis
{
    public static class MediaParser
    {
                ...
                foreach (var media in cExtendedEntities.Media)
                    if (media.Type == "animated_gif" || media.Type == "video")
                    {
                        ...
                        var variant = !variants.Any()
                            ...
                            : variants.First(x => x.Bitrate == variants.Select(y => y.Bitrate).Max());
        ...
        public static IEnumerable<Media> Parse(IEnumerable<TootNet.Objects.Attachment> cAttachments, string cContent)
        {
